    ...
    Lightbox = {
        ...
        initialize: function (options) {
            ...
            this.CloseButton.bind('click', $.bind(this, function () {
                if ($.trim(this.options.callback) !== "") {
                    eval(this.options.callback);
                    Lightbox.options.callback = null;
                }
                this.close();
                return false;
            }));
        ...
        showPopUp: function (caption, url, rel, force, callback, html, onload) {
            this.options.callback = callback;
            this.options.html = html;
            this.options.onload = onload;
            this.show(caption, url, rel, force);
        },
        show: function (caption, url, rel, force) {
            ...
            if (this.ResizeBox)
                this.ResizeBox.unbind('complete'); //fix for jQuery
            ...
            if (params['width'] === 0 && params['height'] === 0) {
                params['width'] = $(window).width() - gap;
                params['height'] = $(window).height() - gap;
            } else {
                if (params['width'] === 0 || params['width'] > ($(window).width() - gap)) {
                    params['width'] = $(window).width() - gap;
                } else {
                    params['width'] = parseInt(params['width']);
                }
                if (params['height'] === 0 || params['height'] > ($(window).height() - gap)) {
                    params['height'] = $(window).height() - gap;
                } else {
            ...
            if (force === 'image' || baseURL.match(imageURL))
            {
                ...
                } else if (url.indexOf('TB_html') !== -1) { //HTML
                    this.ResizeBox.bind('complete', $.bind(this, function () {
                        this.showContent(this.options.html, this.lightbox);
                    }));
                }
        ...
        addButtons: function () {
            if (this.prev)
                this.prev.bind('click', $.bind(this, function (obj, event) {
                    event.preventDefault();
                    this.hook(this.prev);
                }));
            if (this.next)
                this.next.bind('click', $.bind(this, function (obj, event) {
                    event.preventDefault();
                    this.hook(this.next);
                }));
        },
