...
Autocompleter.Base = Class.create({
  baseInitialize: function(element, update, options) {
    ...
    Event.observe(this.update, 'blur',this.hideDiv.bindAsEventListener(this));
...
markPrevious: function() {
	...
	if(overlap.top > threshold) {
		layer.scrollTop = position[1];
  		...
  		if(this.index>8 && this.getEntry(this.index+5)!=null && this.getEntry(this.index+5)!=undefined){
  			this.getEntry(this.index+5).scrollIntoView(false);
  		}else{
  			this.getEntry(this.index).scrollIntoView(false);
  		}
  	}
...
markNext: function() {
	...
	if(overlap.top > threshold) {
		layer.scrollTop = position[1];
		if(this.getEntry(this.index+5)!=null && this.getEntry(this.index+5)!=undefined){
  			this.getEntry(this.index+5).scrollIntoView(false);
  		}else{
  			this.getEntry(this.index).scrollIntoView(false);
  		}
  ...
  onObserverEvent: function() {
    ...
    if(this.element.readOnly == false && (this.getToken()==="%" ||this.getToken()==="%%" || this.getToken().length>=this.options.minChars)) {
    	this.getUpdatedChoices();
    } else {
      this.active = false;
      this.hide();
...
Ajax.Autocompleter = Class.create(Autocompleter.Base, {
  ...
  //alert(encodeUrl);
    var _url = this.url + encodeUrl;
    //alert(_url);
    new Ajax.Request(_url, this.options);
  },
