<%-- 
    Document   : arMigrationReprocessLog
  ...
  <tbody>
    <tr>
      <td class="lightBoxHeader">
          <c:if test="${not empty arMigrationLog}">
            Reprocess Logs - ${arMigrationLog.blNumber}
          </c:if>
      </td>
      <td>
...
<c:choose>
  <c:when test="${not empty arMigrationLog}">
    <div class="scrolldisplaytable" style="height: 180px">
      <table width="100%" cellpadding="0" cellspacing="3" class="displaytagstyleNew" style="border: 1px solid white">
        ...
        <tbody>
          ...
          <c:forEach var="reprocessLog" items="${arMigrationLog.arMigrationReprocessLogs}">
            <tr class="${zebra}">
