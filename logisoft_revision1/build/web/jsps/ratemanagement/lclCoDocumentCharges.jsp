...
String editPath=path+"/lclCoDocumentCharges.do";
String modify="";
...
<html> 
		...
		function disabled(val1)
   {
	...
	<body class="whitebackgrnd" >
		<html:form action="/lclCoDocumentCharges" name="documentChargesForm" type="com.gp.cong.logisoft.struts.ratemangement.form.LclCoDocumentChargesForm" scope="request">
...
<%-- 		 		<html:text property="charge" value="<%=chargecode%>" readonly="true" styleClass="varysizeareahighlightgrey" size="3"/>--%>
	            ...
	            <dojo:autoComplete  formId="documentChargesForm" textboxId="charge" action="<%=path%>/actions/getChargeCode.jsp?tabName=LCL_CODOCUMENT_CHARGES&from=0"/>
 		 		</td>
   		 		<td >
   		 	  <input name="desc"  id="desc" value="<%=codeDesc%>" onkeydown="getdestPort1(this.value)"  size="12"/>
	          <dojo:autoComplete  formId="documentChargesForm" textboxId="desc" action="<%=path%>/actions/getChargeCode.jsp?tabName=LCL_CODOCUMENT_CHARGES&from=1"/>
<%--   		 		<html:text property="desc" value="<%=codeDesc%>"  size="12"/>--%>
   		  		...
   		  		<td align="center">
   		  		<input type="button" value="Add" class="buttonStyleNew" onclick="addDocumentCharges()"/>
        ...
        <display:table name="<%=docList%>" pagesize="<%=pageSize%>"   class="displaytagstyle"  sort="list" id="agsstable" > 
			<%
			    ...
			    {
					LCLColoadDocumentCharges docCharges=(LCLColoadDocumentCharges)docList.get(i);
