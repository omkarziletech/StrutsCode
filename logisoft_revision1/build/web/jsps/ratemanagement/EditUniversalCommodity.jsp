<%@ page language="java" pageEncoding="ISO-8859-1" import="com.gp.cong.logisoft.domain.UniverseCommodityChrg,java.text.DateFormat,java.text.SimpleDateFormat,com.gp.cong.logisoft.beans.AirRatesBean"%>
...
{
defaultRate=(String)session.getAttribute("unibverdefaultRate");
}
...
{
percentage=df.format(lCLColoadCommodityCharges.getPercentage());
}
...
<html> 
	<head>
		...
		function disabled(val1,val2,val3) {
			if(val1!=""&&(val1 == 0 || val1== 3))
			{
		   		...
		   		var input = document.getElementsByTagName("input");
			   		...
			   		{
			  		if(input[i].name=="asFrfgted" || input[i].name=="standard")
	 				{
	  	 		...
	  	 		var imgs = document.getElementsByTagName('img');
		  	 		...
		  	 		document.editUniversalCommodityForm.standard.disabled=true
		...
		function addAGSS()
		{
		document.editUniversalCommodityForm.buttonValue.value="add";
		document.editUniversalCommodityForm.submit();
		}
		function delete1()
		{
			document.editUniversalCommodityForm.buttonValue.value="delete";
   			document.editUniversalCommodityForm.submit();
   		}
   		function cancelbtn()
		{
			document.editUniversalCommodityForm.buttonValue.value="cancel";
   			document.editUniversalCommodityForm.submit();
		}
		 </script>
	</head>
	<body class="whitebackgrnd" onLoad="disabled('<%=modify%>','<%=message%>','<%=exclude%>')">
		<html:form action="/editUniversalCommodity" scope="request">
<div align="left">
  ...
  <tr> 
    <td>
		...
		<tr class="textlabels">
         ...
         {
         %>
         <td><html:text property="amount" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8"   styleId="amount" value="<%=amount%>"/></td>
         <% 
         }
         if(chargeType!=null&&chargeType.equals("Weight or measure"))
         {
         ...
         <%if(defaultRate.equals("E"))
         { %>
         <td><html:text property="amtPerCft" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8"  styleId="amtPerCft" value="<%=cft%>"/></td>
  		 <td><html:text property="amtPer100lbs" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8"  styleId="amtPer100lbs" value="<%=lbs%>"/></td>
  		 <%}else if(defaultRate.equals("M"))
  		 { %>
  		 <td><html:text property="amtPerCbm" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8"  styleId="amtPerCbm" value="<%=cbm%>"/></td>
  		 <td><html:text property="amtPer1000kg" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8"  styleId="amtPer1000kg" value="<%=amtkg%>"/></td>	
  		 <%} %>
  		 <td><html:text property="minAmt" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8"  styleId="minAmt" value="<%=minAmt%>"/></td>
         <%
         }
         if(chargeType!=null&&chargeType.equals("Charge percent"))
         {
         %>
         <td><html:text property="percentage" onkeypress="check(this,2,event)" onblur="checkdec(this)" maxlength="5" size="5"  value="<%=percentage%>"/></td>
		 <td><html:text property="minAmt" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8"  styleId="minAmt" value="<%=minAmt%>"/></td>
		 ...
		 %>
		  ...
		  <td><html:text property="insuranceRate" onkeypress="check(this,3,event)" onblur="checkdec(this)" maxlength="6" size="6" styleId="minAmt" value="<%=insuranceRate%>"/></td>
		  <td><html:text property="insuranceAmt" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8"  styleId="minAmt" value="<%=insuranceAmt%>"/></td>
		  <td><html:text property="minAmt" onkeypress="check(this,5,event)" onblur="checkdec(this)" maxlength="8" size="8" styleId="minAmt" value="<%=minAmt%>"/></td>
		 <%
		 }
		 %>
