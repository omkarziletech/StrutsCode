<table>
    <tr>
        <td>
            ...
            <input type="button" value="Save" class="buttonStyleNew" id="saveFclBl" onmouseover="tooltip.show('<strong>${mandatoryFieldForBl}</strong>',null,event);" onmouseout="tooltip.hide();" onclick="setAction('saveFclBl')"/>
    ...
    <c:choose>
        ...
        <c:otherwise>
            ...
            <c:if test="${fn:indexOf(fclBlForm.fclBl.bolId,'==')<0}">
                ...
                id="correctionButton" onclick="return GB_show('FclBlCorrections','${path}/fclBlCorrections.do?buttonValue=FclBl&blNumber=${fclBlForm.fclBl.bolId}&fileNo=${fclBlForm.fclBl.fileNo}',500,1100);"/>
            </c:if>
        </c:otherwise>
    </c:choose>
    <c:if test="${fn:indexOf(fclBlForm.fclBl.fileNo,'-')<0}">
        <input type="button" value="Create Multi BL File" class="buttonStyleNew" onclick="createMultipleBL()" id="multipleBL"/>
    </c:if>
    ...
    <input type="button" value="Note" class="${manualNotesCount}" 
           onclick="return GB_show('Notes','${path}/notes.do?moduleId=FILE&moduleRefId=${fclBlForm.fclBl.fileNo}',350,750);" id="note"/>
    ...
    id="aesbutton" onclick="GB_show('AES/ITN Details','${path}/fclAESDetails.do?buttonValue=fclbl&bolid=${fclBlForm.fclBl.bol}&fileNo=${fclBlForm.fclBl.fileNo}&ready=${fclBlForm.fclBl.readyToPost}&importFlag=${fclBlForm.fclBl.importFlag}',450,900);"/>
    ...
    id="inbond" onclick="GB_show('Inbond Details','${path}/fclInbondDetails.do?buttonValue=fclbl&bolid=${fclBlForm.fclBl.bol}&fileNo=${fclBlForm.fclBl.fileNo}',320,950);"/>
    ...
    id="scanAttach" onclick="scanOrAttach()"/>
    <input type="button" value=" AR Invoice" id="arInvoice" class="buttonStyleNew" onclick="return GB_show('AR Invoice', '${path}/arRedInvoice.do?action=listArInvoice&fileNo=${fclBlForm.fclBl.fileNo}&screenName=BL&fileType=${fclBlForm.fclBl.importFlag}&voyInternal=${fclBlForm.fclBl.voyageInternal}', 550,  1100)"/>
    <input type="button" value="Send EDI" id="sendEdi" style="visibility:  hidden" class="buttonStyleNew" onclick="generate304Xml()"/>
