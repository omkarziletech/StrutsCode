...
<c:set var="selectedMenu" value="${param.selectedMenu}"></c:set>
    ...
    <div style="height: 170px;" class="result-container">
        ...
        <c:forEach items="${invoiceChargeList}" var="arInvoiceCharge">
            ...
            <tr class="${zebra}">
                ...
                <td>
                    <c:choose>
                        <c:when test="${selectedMenu eq 'Imports'}">
                            <c:if test="${not empty arInvoiceCharge.lclDrNumber}">
                                ${arInvoiceCharge.lclDrNumber}
                            </c:if>
                        </c:when>
                        <c:otherwise>     
                            ${fileNumber}
                ...
                <td>
                    <c:if test="${status ne 'AR' && empty arInvoiceCharge.lclDrNumber}">
                        <img src="${path}/images/error.png" alt="delete" height="12px" title="Delete Charge" width="12px"  style="cursor: pointer"
                             onclick="deleteInvoiceCharge('deleteInvoiceCharge', '#lclVoyageArInvoiceForm', '${arInvoiceCharge.id}', '#redInvoiceCharge');"/>
                    </c:if>
                </td>
            </tr>
        </c:forEach>
