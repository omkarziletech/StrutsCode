jQuery.autocomplete = function(input, options) {
    ...
    $results.hide().addClass(options.resultsClass).css("position", options.resultPosition);
	...
	if (options.showLeft){
	    left = (offset.left + $input.width() - iWidth) + "px"
	}
	$results.css({
	...
	if(options.scroll) {
		...
		if (!jQuery(event.target).closest(".ac_results_ul li" ).length) {
			...
			jQuery(document).one('mousedown', function(event) {
			    if (jQuery(event.target) !== $input && jQuery(event.target) !== menuElement
				&& !jQuery(event.target).parent().hasClass("ac_results")) {
				$input.blur();
			    }
			});
		    }, 1 );
...
jQuery.fn.autocomplete = function(url, options, data) {
    ...
    options.showLeft = options.showLeft || false;
    this.each(function() {
	var input = this;
