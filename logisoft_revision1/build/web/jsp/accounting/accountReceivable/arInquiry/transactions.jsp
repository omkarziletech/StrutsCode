...
<html>
	...
	<table class="table" style="margin: 0;border: none;padding: 0;">
	    <tr>
		<th class="toggle" onclick="toggle('posted');">Posted Transactions</th>
	    </tr>
	    <tr>
		<c:choose>
	    ...
	    <c:if test="${not empty unpostedTransactions}">
		<tr>
		    <th class="toggle" onclick="toggle('un-posted');">Un-posted Transactions</th>
		</tr>
		<tr>
		    <td>
			<div class="result-container un-posted">
			    <table width="100%" cellpadding="0" cellspacing="1" class="display-table">
				<thead>
				    <tr>
					<th class="width-145px">Transaction Date</th>
				...
				<tbody>
				    ...
				    <c:set var="totalAmount" value="0"/>
				    <c:set var="totalAdjustment" value="0"/>
				    <c:forEach var="row" items="${unpostedTransactions}">
					<tr class="${zebra}">
					    <td>${row.transactionDate}</td>
					    <td>${row.postedDate}</td>
					    <td>${row.type}</td>
