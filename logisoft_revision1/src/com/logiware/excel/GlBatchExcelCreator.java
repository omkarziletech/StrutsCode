...
public class GlBatchExcelCreator extends BaseExcelGenerator {

    private WritableCellFormat mainHeaderCell = null;
    private WritableCellFormat grayHeaderCell = null;

    private void writeContents(Batch batch) throws Exception {
        this.writableSheet = this.writableWorkbook.createSheet(batch.getBatchId().toString(), 0);
        int row = 0;
        ...
        this.writableSheet.addCell(new Label(0, row, "Batch ", this.mainHeaderCell));
        this.writableSheet.addCell(new Number(1, row, batch.getBatchId(), this.noBorderCellBlackLeft));
        this.writableSheet.addCell(new Label(2, row, "Description ", this.mainHeaderCell));
        this.writableSheet.mergeCells(3, row, 5, row);
        this.writableSheet.addCell(new Label(3, row, batch.getBatchDesc(), this.noBorderCellBlackLeft));
        row++;
        this.writableSheet.addCell(new Label(0, row, "Debit ", this.mainHeaderCell));
        this.writableSheet.addCell(new Number(1, row, batch.getTotalDebit(), this.noBorderNumberCellBlackLeft));
        this.writableSheet.addCell(new Label(2, row, "Credit ", this.mainHeaderCell));
        this.writableSheet.addCell(new Number(3, row, batch.getTotalCredit(), this.noBorderNumberCellBlackLeft));
        ...
        if ((null != batch.getJournalEntrySet()) && (!batch.getJournalEntrySet().isEmpty())) {
            TreeSet<JournalEntry> journalEntries = new TreeSet(new JournalEntryComparator());
            journalEntries.addAll(batch.getJournalEntrySet());
            for (JournalEntry journalEntry : journalEntries) {
                row++;
                this.writableSheet.mergeCells(0, row, 5, row);
                this.writableSheet.addCell(new Label(0, row, "", this.grayHeaderCell));
                row++;
                ...
                row++;
                this.writableSheet.addCell(new Label(0, row, "Debit ", this.mainHeaderCell));
                this.writableSheet.addCell(new Number(1, row, journalEntry.getDebit(), this.noBorderNumberCellBlackLeft));
                this.writableSheet.addCell(new Label(2, row, "Credit ", this.mainHeaderCell));
                this.writableSheet.addCell(new Number(3, row, journalEntry.getCredit(), this.noBorderNumberCellBlackLeft));
                row++;
                ...
                if ((null != journalEntry.getLineItemSet()) && (!journalEntry.getLineItemSet().isEmpty())) {
                    ...
                    for (LineItem lineItem : lineItems) {
                        ...
                        this.writableSheet.addCell(new Number(2, row, lineItem.getDebit(), this.noBorderNumberCellBlackRight));
                        this.writableSheet.addCell(new Number(3, row, lineItem.getCredit(), this.noBorderNumberCellBlackRight));
    ...
    public String exportToExcel(Batch batch) throws Exception {
        ...
        fileName.append("GL_Batch_").append(batch.getBatchId()).append(".xls");
