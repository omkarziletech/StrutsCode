...
public class AccrualMigrationJob implements org.quartz.Job {
    ...
    public static void run() throws Exception {
        try {
            ...
            loadAccrualsToLogiware.reprocessAllErrors();
            //loadAccrualsToLogiware.loadMissingAccruals();
        } catch (Exception e) {
            throw e;
        }
    }
    ...
    public void execute(JobExecutionContext jec) throws JobExecutionException {
        ...
        try {
            log.info("Accrual Migration Job started on " + new Date());
            transaction = dao.getCurrentSession().getTransaction();
            if (!transaction.isActive()) {
                transaction.begin();
            }
            Job job = dao.findByClassName(AccrualMigrationJob.class.getCanonicalName());
            job.setStartTime(new Date());
            run();
            job.setEndTime(new Date());
            transaction.commit();
            log.info("Accrual Migration Job ended on " + new Date());
        } catch (Exception e) {
            log.info("Accrual Migration Job failed on " + new Date(), e);
            try {
                Thread.sleep(5000);
                log.info("Accrual Migration Job restarted on " + new Date());
                ...
                log.info("Accrual Migration Job failed again on " + new Date(), ex);
