...
public class LclExportOldPdfFormat extends LclReportFormatMethods {
    ...
    private void init(String outputFileName) throws Exception {
        ...
        LclExportOldPdfFormat.TableHeader event = new LclExportOldPdfFormat.TableHeader();
        String fileIds = new ExportUnitQueryUtils().getPickedCargoBkgList(lclUnitSs.getId());
        pdfWriter.setPageEvent(event);
        document.open();
        Map<String, String> orderMap = new TreeMap<String, String>();

        if (null != fileIds && !fileIds.equalsIgnoreCase("")) {
            int count = 0;
            totalDrCount = fileIds.split(",").length;
            ...
            for (Map.Entry<String, String> map : orderMap.entrySet()) {
                drCount++;
                document.add(commonDrList(map.getValue()));
                if (drCount == 5) {
    ...
    private PdfPTable writeheader() throws Exception {
        ...
        headerCell.setPaddingTop(-3f);
        p = new Paragraph(5f, " Loading Deadline . . :" + loadingdate, blackBoldFontSize6);
        headerCell.addElement(p);
        headerTable.addCell(headerCell);
    ...
    public PdfPTable commonDrList(String fileId) throws DocumentException, Exception {
        Paragraph p = null;
        PdfPTable subheaderTable = new PdfPTable(9);
        subheaderTable.setWidths(new float[]{2.2f, 0.5f, 1.5f, 1.5f, 2f, 1f, 6.05f, 6f, 1.95f});
        subheaderTable.setWidthPercentage(100f);
