...
public class LclLoadPdfCreator extends LclReportFormatMethods {

    private static final Logger log = Logger.getLogger(LclExportOldPdfFormat.class);
    private LclSsHeader ssHeader;
    ...
    public LclLoadPdfCreator(LclUnitSs unitSs, LclSsHeader ssHeader) throws Exception {
        this.ssHeader = ssHeader;
        destination.append(ssHeader.getDestination().getCountryId().getCodedesc());
        destination.append(", ").append(ssHeader.getDestination().getUnLocationName());
        origin.append(ssHeader.getOrigin().getUnLocationName());
        if (null != ssHeader.getOrigin().getStateId()
                && CommonUtils.isNotEmpty(ssHeader.getOrigin().getStateId().getCode())) {
            origin.append(", ").append(ssHeader.getOrigin().getStateId().getCode());
        }
        if (CommonUtils.isNotEmpty(unitSs.getSpBookingNo())) {
            masterBkgNO = unitSs.getSpBookingNo().toUpperCase();
        }

        LclSsDetail ssDetail = ssHeader.getVesselSsDetail();
        if (ssDetail != null) {
            if (CommonFunctions.isNotNull(ssDetail.getSpAcctNo())) {
                ssLine = ssDetail.getSpAcctNo().getAccountName();
            }
            ...
            if (CommonFunctions.isNotNull(ssDetail.getStd())) {
                sailingDate = (DateUtils.parseDateToString(ssDetail.getStd()));
            }
            if (CommonFunctions.isNotNull(ssDetail.getSpReferenceNo())) {
                sslinevoyage = (ssDetail.getSpReferenceNo());
            }
            if (CommonFunctions.isNotNull(ssDetail.getGeneralLrdt())) {
                loadingdate = DateUtils.parseDateToString(ssDetail.getGeneralLrdt());
            }
            if (CommonFunctions.isNotNull(ssDetail.getHazmatLrdt())) {
                hazmetDate = DateUtils.parseDateToString(ssDetail.getHazmatLrdt());
            }
        }
        if (CommonFunctions.isNotNull(unitSs)) {
            unitsNumber.append(unitSs.getLclUnit().getUnitNo()).append("      ");
            unitsNumber.append(new LclUtils().getContainerSize(unitSs.getId()));
            locationDoor = new LclUnitWhseDAO().getLocationValues(unitSs.getLclUnit().getId(),
                    unitSs.getLclSsHeader().getId());
            if (CommonFunctions.isNotNull(unitSs.getSUHeadingNote())) {
                sealNo = unitSs.getSUHeadingNote();
            }
            LclUnitWhse unitWhse = new LclUnitWhseDAO().getLclUnitWhseFirstRecord(unitSs.getLclUnit().getId(),
                    unitSs.getLclSsHeader().getId());
    ...
    class TableHeader extends PdfPageEventHelper {
        ...
        public void onStartPage(PdfWriter writer, Document document) {
            try {
                ...
                log.info("Exception on class Loaded Print Selection View in method onStartPage" + new Date(), ex);
            }
        }
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            try {
                ...
                log.info("Exception on class Loaded Print Selection View in method onEndPage" + new Date(), ex);
            }
        }
    ...
    private void init(String outputFileName, String fileId) throws Exception {
        ...
        LclLoadPdfCreator.TableHeader event = new LclLoadPdfCreator.TableHeader();
        ...
        String secure_list = exportQueryUtils.getSecureCargoBkgList(null, ssHeader.getServiceType(), fileList);
        totalDrCount += null != secure_list ? secure_list.split(",").length : 0;
        drListMap.put("secure", null != secure_list ? secure_list : "");

        String upscargo_list = exportQueryUtils.getUPSCargoBkgList(null, ssHeader.getServiceType(), fileList);
        totalDrCount += null != upscargo_list ? upscargo_list.split(",").length : 0;
        drListMap.put("ups", null != upscargo_list ? upscargo_list : "");

        String general_list = exportQueryUtils.getGeneralCargoBkgList(null, ssHeader.getServiceType(), fileList);
        totalDrCount += null != general_list ? general_list.split(",").length : 0;
        drListMap.put("general", null != general_list ? general_list : "");

        String hazmatcargo_list = exportQueryUtils.getHazmatCargoBkgList(null, ssHeader.getServiceType(), fileList);
        ...
        if ("E".equalsIgnoreCase(ssHeader.getServiceType())) {
            intrCargoName = "**INTR CARGO***";
            arriveOrCargo_list = exportQueryUtils.getINTRCargo(ssHeader.getOrigin().getId(),
                    ssHeader.getDestination().getId(), null, fileList);
        } else if ("N".equalsIgnoreCase(ssHeader.getServiceType())) {
            intrCargoName = "*TO ARRIVE*****";
            arriveOrCargo_list = exportQueryUtils.toArriveBookingList(null, ssHeader.getOrigin().getId(),
                    ssHeader.getDestination().getId(), fileList);
        }
