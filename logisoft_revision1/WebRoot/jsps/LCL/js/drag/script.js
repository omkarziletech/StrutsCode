...
redips_init = function () {
    ...
    rd.myhandler_row_dropped = function () {
        ...
        var percent = 0;
        var wPercent = 0;
        var fileNo = $(rd.obj_old).closest('tr').find('td:eq(1)').text();
        var vol = $(rd.obj_old).closest('tr').find('td:eq(7)').text();
        var weight = $(rd.obj_old).closest('tr').find('td:eq(6)').text();
        if (weight == '') {
            weight = 0;
        }
        if (vol == '') {
        ...
        var alloctedWeight = $('#weightAllocated').val();
        var allocatedVol = $('#volumeAllocated').val();
        var totalVol = $('#unitVolume').val();
        var totalWeight = $('#unitWeight').val();
        var v = totalVol;
        var w = totalWeight;
        $.ajaxx({
            dataType: "json",
        ...
        if (pos[0] == 1) {
            v = parseFloat(allocatedVol) + parseFloat(vol);
            w = parseFloat(alloctedWeight) + parseFloat(weight);
        }
        else {
            v = parseFloat(allocatedVol) - parseFloat(vol);
            w = parseFloat(alloctedWeight) - parseFloat(weight);
        }
        $('#weightAllocated').val(w);
        $('#volumeAllocated').val(v);
        percent = (v / parseFloat(totalVol)) * 100;
        wPercent = (w / parseFloat(totalWeight)) * 100;
        percent = Math.round(parseFloat(percent) * Math.pow(10, 2)) / Math.pow(10, 2);
        wPercent = Math.round(parseFloat(wPercent) * Math.pow(10, 2)) / Math.pow(10, 2);
        $('#progress-volume').progressbar('setValue', percent);
        $('#progress-weight').progressbar('setValue', wPercent);
