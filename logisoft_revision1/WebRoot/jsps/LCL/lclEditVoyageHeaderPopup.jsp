<%@include file="init.jsp"%>
<%@include file="/taglib.jsp"%>
<%@include file="colorBox.jsp" %>
<%@include file="/jsps/preloader.jsp" %>
<%@include file="../includes/baseResources.jsp" %>
<%@include file="/jsps/includes/jspVariables.jsp" %>
<jsp:useBean id="lclAddVoyageForm" scope="request" class="com.gp.cvst.logisoft.struts.form.lcl.LclAddVoyageForm"/>
<cong:javascript src="${path}/js/common.js"></cong:javascript>
<cong:form  action="/lclAddVoyage" name="lclAddVoyageForm" id="lclAddVoyageForm" >
    <cong:hidden id="methodName" name="methodName"/>
    <cong:hidden id="headerId" name="headerId" />
    <cong:table width="100%" style="margin:5px 0; float:left">
        <cong:tr>
            <cong:td styleClass="tableHeadingNew" colspan="4">
                <cong:div styleClass="floatLeft">Edit Header</cong:div>
            </cong:td>
        </cong:tr>
    </cong:table>
    <cong:table width="90%">
        <cong:tr>
         
                <cong:td styleClass="style2" align="right" valign="top" width="10%">Voyage#</cong:td>
                <cong:td valign="top" width="5%">
                    <cong:text name="scheduleNo" id="scheduleNo" readOnly="true"  styleClass="text readOnly"/>
                </cong:td>
                <cong:td styleClass="textlabelsBoldforlcl">Hazardous</cong:td>
                <cong:td  >
                    <cong:radio value="Y" name="hazmatPermitted" container="NULL" /> Yes
                    <cong:radio value="N" name="hazmatPermitted" container="NULL" /> No
                </cong:td>
                <cong:td styleClass="textlabelsBoldforlcl">Refrigeration</cong:td>
                <cong:td>
                    <cong:radio value="Y" name="refrigerationPermitted"  container="NULL" /> Yes
                    <cong:radio value="N" name="refrigerationPermitted"  container="NULL" /> No
                </cong:td>
                <cong:td styleClass="style2" align="right" width="10%">Remarks</cong:td>
                <cong:td width="10%">
                    <cong:textarea cols="5" rows="15" styleClass="refusedTextarea textlabelsBoldForTextBox " id="remarks"
                                   name="remarks" value="${lclAddVoyageForm.remarks}" >
                    </cong:textarea>
                </cong:td>
        </cong:tr>
        <cong:tr><cong:td colspan="8"></cong:td></cong:tr>
    </cong:table>
    <cong:table width="80%">
        <cong:tr>
            <cong:td styleClass="style2" align="right" width="10%"></cong:td>
            <cong:td width="5%">

            </cong:td>
            <cong:td styleClass="style2" align="right" width="10%"></cong:td>

            <cong:td width="10%">
                <div class="button-style1" onclick="saveVoyageHeader();">Save Voyage Header</div>
            </cong:td>
            <cong:td styleClass="style2" align="right" width="5%"></cong:td>
            <cong:td width="10%">

            </cong:td>

        </cong:tr>
    </cong:table>
</cong:form>
<script type="text/javascript">
    function saveVoyageHeader(){
        showLoading();
        parent.document.getElementById("hazmatPermitted").value = $('input:radio[name=hazmatPermitted]:checked').val();
        parent.document.getElementById("hiddenRefrigerationPermitted").value = $('input:radio[name=refrigerationPermitted]:checked').val();
        parent.document.getElementById("remarks").value = document.getElementById("remarks").value;
        parent.$("#methodName").val('saveVoyageHeader');
        parent.$("#lclAddVoyageForm").submit();
    }
</script>