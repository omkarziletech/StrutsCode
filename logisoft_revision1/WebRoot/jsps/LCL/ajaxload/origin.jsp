...
<c:choose>
    <c:when test="${originQuery=='RELAYORIGIN'}">
        <cong:td>
            <cong:autocompletor id="portOfOriginR" name="portOfOrigin" callback="filldeliverCargo();submitAjaxFormforOrigin('lclRelayFind','#lclBookingForm','#polPod','');"
                                ...
                                width="250" container="NULL" paramFields="finalDestinationId" value="${lclBookingForm.portOfOrigin}" shouldMatch="true"/>
        </cong:td>
    </c:when>
    <c:otherwise>
        <cong:td>
            <cong:autocompletor id="portOfOriginR" name="portOfOrigin" callback="filldeliverCargo();submitAjaxFormforOrigin('lclRelayFind','#lclBookingForm','#polPod','');"
                                template="${tempName}" fields="NULL,NULL,originUnlocationCode,portOfOriginId" query="${originUnLocQuery}" styleClass="${requiredField} textlabelsBoldForTextBox"
                                width="250" container="NULL" value="${lclBookingForm.portOfOrigin}" shouldMatch="true"/>
        </cong:td>
    </c:otherwise>
</c:choose>
